...
if st.session_state.track_previews:
    ...
    # 🎯 修正 1：新增一個隱藏的快取，用來偵測「批量輸入框」有沒有變動
    if "prev_bulk_album" not in st.session_state:
        st.session_state.prev_bulk_album = ""

    bulk_album = st.text_input(
        "統一設定這批歌曲的專輯（留空則預設為各自的歌名）", 
        placeholder="例如：偷偷藏不住ost"
    )

    # 檢查批量欄位有沒有被使用者改過
    bulk_changed = bulk_album != st.session_state.prev_bulk_album
    if bulk_changed:
        st.session_state.prev_bulk_album = bulk_album
    ...
    for index, preview in enumerate(previews):
        ...
        with container:
            ...
            with form_col:
                ...
                # 歌曲標籤下拉選單
                type_options = ["無", "主題曲", "片尾曲", "插曲", "推廣曲", "配樂"]
                # 確保重新渲染時能抓到原本選的項目
                default_type_idx = type_options.index(preview.song_type) if preview.song_type in type_options else 0
                
                song_type = st.selectbox(
                    "歌曲標籤 (會自動加入歌名後綴與 iTunes 副標題)",
                    options=type_options,
                    index=default_type_idx,
                    key=f"preview_type_{index}_{preview.url}",
                )
                ...
                # 🎯 修正 2：動態控管專輯輸入框的 Session State 生命週期
                album_key = f"preview_album_{index}_{preview.url}"
                song_changed_key = f"last_seen_song_{index}_{preview.url}"
                
                # 初始化歌名偵測快取
                if song_changed_key not in st.session_state:
                    st.session_state[song_changed_key] = preview.song

                # --- 核心邏輯判斷 ---
                if bulk_album.strip():
                    # 情況 A：如果使用者剛輸入了「批量專輯」，強制把所有人的快取改成批量專輯
                    if bulk_changed or album_key not in st.session_state:
                        st.session_state[album_key] = bulk_album.strip()
                else:
                    # 情況 B：批量專輯是空的
                    # 如果是「初次載入」、「批量剛被清空」、或是使用者「手動改了歌名」，就讓專輯名稱同步等於歌名
                    if (album_key not in st.session_state or 
                        bulk_changed or 
                        song != st.session_state[song_changed_key]):
                        
                        st.session_state[album_key] = song
                        st.session_state[song_changed_key] = song  # 更新最後看到的歌名

                # 已經在上面強制修正過 state 了，這裡直接綁定 key 即可，不用再傳 value
                album = st.text_input(
                    "專輯",
                    key=album_key,
                )
            ...
            # 🎯 變動位置 3：將使用者最終確認/編輯的 album 帶入封裝物件中
            confirmed_tracks.append(
                SongMetadata(
                    url=preview.url,
                    song=song.strip() or preview.song,
                    artist=artist.strip() or preview.artist,
                    album=album.strip() or default_album,  # 帶入專輯資料
                    song_type=song_type,
